...
namespace DevSocket
{
    public class DevReceiveFilter: SubFixedHeaderReceiveFilter<BinaryRequestInfo>
    {
        ...
        protected override int GetBodyLengthFromHeader(byte[] header, int offset, int length)
        {
            ...
            else
            {
                ...
                int len = BitConverter.ToUInt16(header, offset + 1) + 8 + 1 + 1 + 1 + 1;
                Share.Instance.WriteLog(len.ToString());
                Share.Instance.WriteLog(header[offset].ToString());
                return len;
            }
        }
        ...
        protected override BinaryRequestInfo ResolveRequestInfo(ArraySegment<byte> header, byte[] bodyBuffer, int offset, int length)
        {
            return AspectF.Define.Retry(Share.Instance.CatchExpection)
                .Return<BinaryRequestInfo>(() =>
                {
                    ...
                    Share.Instance.WriteLog("Receive from Dev : " + ByteHelper.ByteToHexStrWithDelimiter(data, " ", false));
