...
namespace Utility
{
    ...
    public static class AspectExtensions
    {
        ...
        [DebuggerStepThrough]
        public static AspectF StringMustBeNonNull(this AspectF aspect, params string[] args)
        {
            return aspect.Combine((work) =>
            {
                for (int i = 0; i < args.Length; i++)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format("IP at index {0} is null", i));
                    }
                }
                work();
            });
        ...
        public static AspectF Log(this AspectF aspect, Action<string> LogWriter, string beforeMessage, string afterMessage)
        {
            return aspect.Combine((work) =>
            {
                ...
                //logWriter.Info(DateTime.Now.ToUniversalTime().ToString());
                //logWriter.Info('\t');
                //logWriter.Info(beforeMessage);
                //LFI_Log.Info(beforeMessage);
                LogWriter(beforeMessage);
                //logWriter.Info(Environment.NewLine);
