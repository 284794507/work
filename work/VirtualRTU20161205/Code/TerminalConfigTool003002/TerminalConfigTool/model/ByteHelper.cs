...
namespace CommunicateCore.Utility
{
    public sealed class ByteHelper
    {
        ...
        /// <returns></returns>
        public static string ByteToHexStrWithDelimiter(byte[] bytes, string separator, bool needReverse)
        {
            string returnStr = "";
            if (bytes != null)
            {
        ...
        public static DateTime BytesToDataTime_5(byte[] bytes)
        {
            ...
            if(bytes.Length==6)
            {
                try
                {
                    int year = Convert.ToInt16(bytes[0].ToString("X2")) * 100 + Convert.ToInt16(bytes[1].ToString("X2"));
                    int month = Convert.ToInt16(bytes[2].ToString("X2"));
                    int day = Convert.ToInt16(bytes[3].ToString("X2"));
                    int hour = Convert.ToInt16(bytes[4].ToString("X2"));
                    int minute = Convert.ToInt16(bytes[5].ToString("X2"));

                    dt = new DateTime(year, month, day, hour, minute, 0);
                }
                catch(Exception ex)
                {
                    throw new Exception("6字节转换时间出错:"+ex.Message);
                }
            }
        ...
        public static DateTime BytesToDateTime(Byte[] bytes)
        {
            ...
            if (bytes.Length == 6 || bytes.Length == 7)
            {
                ...
                try
                {
                    int second = Convert.ToInt16(bytes[0].ToString("X2"));
                    int minute = Convert.ToInt16(bytes[1].ToString("X2"));
                    int hour = Convert.ToInt16(bytes[2].ToString("X2"));
                    int day = Convert.ToInt16(bytes[3].ToString("X2"));
                    int month = Convert.ToInt16(bytes[4].ToString("X2"));
                    //老协议 42 05 15 20 05 09 新协议 42 05 15 20 05 09 20 取年份其实都是20xx年
                    int year = 2000 + Convert.ToInt16(bytes[5].ToString("X2"));
        ...
        public static DateTime Bytes7ToDateTime_Plus(Byte[] bytes, bool dateType)
        {
            ...
            if (bytes.Length == 7)
            {
                ...
                try
                {
                    ...
                    {
                        int second = Convert.ToInt16(bytes[6].ToString("X2"));
                        int minute = Convert.ToInt16(bytes[5].ToString("X2"));
                        int hour = Convert.ToInt16(bytes[4].ToString("X2"));
                        int day = Convert.ToInt16(bytes[3].ToString("X2"));
                        int month = Convert.ToInt16(bytes[2].ToString("X2"));
                        int year = 2000 + Convert.ToInt16(bytes[1].ToString("X2"));
                        dt = new DateTime(year, month, day, hour, minute, second);
                    }
                }
                catch (Exception e)
        ...
        /// <summary>
        /// 根据DateTime 返回6个字节的数组(YYMMHHmmss)
        /// </summary>
        /// <param name="dt"></param>
        /// <returns></returns>
        public static Byte[] DateTimeToBytes_6(DateTime dt)
        {
            Byte[] bytes = new Byte[6];
            ...
            {
                bytes[0] = 20;
                bytes[1] = Convert.ToByte((dt.Year - 2000).ToString(), 16);
                bytes[2] = Convert.ToByte(dt.Month.ToString(), 16);
                bytes[3] = Convert.ToByte(dt.Day.ToString(), 16);
                bytes[4] = Convert.ToByte(dt.Hour.ToString(), 16);
                bytes[5] = Convert.ToByte(dt.Minute.ToString(), 16);
            }
            catch (Exception e)
            {
                throw new Exception("DateTime转换成Bytes出错：" + e.Message);
        ...
        public static Byte[] DateTimeToBytes_7(DateTime dt)
        {
            ...
            try
            {
                bytes[6] = Convert.ToByte(dt.Second.ToString(), 16);
                bytes[5] = Convert.ToByte(dt.Minute.ToString(), 16);
                bytes[4] = Convert.ToByte(dt.Hour.ToString(), 16);
                bytes[3] = Convert.ToByte(dt.Day.ToString(), 16);
                bytes[2] = Convert.ToByte(dt.Month.ToString(), 16);
                bytes[1] = Convert.ToByte((dt.Year - 2000).ToString(), 16);
                bytes[0] = Convert.ToByte("20", 16);
        ...
        public static Byte[] CtuAddrToBytes(string ctuAdd)
        {
            ...
            try
            {
                int value = Convert.ToInt32(ctuAdd.Trim());
        ...
        public static string bytesToCtuAddrString_8(Byte[] bytes)
        {
            ...
            result = BitConverter.ToInt64(bytes, 0).ToString();
