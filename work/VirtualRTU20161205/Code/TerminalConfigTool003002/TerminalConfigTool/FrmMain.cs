    ...
    public partial class FrmMain : Form
    {
        ...
        private void AcceptConnect()
        {
            AspectF.Define.Retry()
                .Do(() =>
                {
                    ...
                    TcpServer.Start(TerminalShare.GetShare.MaxTcpNum);
                    //while (true)
                    {
                        //allDone.Reset();
        ...
        private void AcceptTcpClient(IAsyncResult ar)
        {
            AspectF.Define.Retry().
                Do(() =>
                {
                    ...
                    if (TerminalShare.GetShare.ClientList.Count ==null || TerminalShare.GetShare.ClientList.Count <= TerminalShare.GetShare.MaxTcpNum)
                    {
            ...
            AspectF.Define.Retry().
                Do(() =>
                {
                    ...
                    if (terminal.CurTcpClient.Connected)
                    {
                        ...
                        {
                            return;
                            TerminalShare.GetShare.WriterLog("设备已退出！");
                        }
                        int len = 0;
                        try
                        {
        ...
        private void btnSetGateway_Click(object sender, EventArgs e)
        {
            ...
            AspectF.Define.Retry()
                ...
                .Do(() =>
                {
                    ...
                    info.CfgNum = 5;
                    info.CfgDetail = new ConfigDetail[5];
        ...
        private byte[] GetMacByString(string mac)
        {
            ...
            if (arr.Length == 6)
            {
                for (int i = 0; i < 6; i++)
                {
                    result[i] = Convert.ToByte(arr[i].ToString(), 16);
                }
            }
        ...
        private void btnStartUpgrade_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                if (chkListDevAddr.GetItemChecked(i))
                {
                    ...
                    if (TerminalShare.GetShare.ClientList.ContainsKey(addr))
                    {
                        ...
                        if(curClient.SVer!= TerminalShare.GetShare.NewSVer)
                        {
                            curClient.CurUpgradeInfo = new UpgradeInfo();
            ...
            if(MessageBox.Show("是否对已选择的"+ TerminalShare.GetShare.UpgradeList.Count+ "个设备进行升级？","询问",MessageBoxButtons.OKCancel)==DialogResult.OK)
            {
                //btnStartUpgrade.Enabled = false;
                foreach (KeyValuePair<string, TerminalClient> kvp in TerminalShare.GetShare.UpgradeList)
                {
                    kvp.Value.CurUpgradeInfo = null;
                ...
                {                    
                    ...
                    if (isSend)
                    {
                        ...
                        if (curClient.CurUpgradeInfo.Stage == 3)
                        {
                            ...
                            {
                                TerminalShare.GetShare.WriterLog("upgrade success: " + curClient.Addr+ "，总包数："+curClient.CurUpgradeInfo.TotalNum + "，成功包数：" + curClient.CurUpgradeInfo.SendNo);
                                TerminalShare.GetShare.UpgradeList.Remove(curClient.Addr);

                                string strFilePath = Path.Combine(Environment.CurrentDirectory, "upgrade.txt");
                                if(TerminalShare.CheckLogFile(curClient.Addr, strFilePath))
                                {
                                    TerminalShare.OpenLogFile(strFilePath);
                                    TerminalShare.WriteLogFile(curClient.Addr);
                                    TerminalShare.CloseLogFile();
                                }
                            }
                            if (TerminalShare.GetShare.UpgradeList.Count == 0)
                            {
                                btnStartUpgrade.Enabled = true;
                    ...
                    delegate
                    {
                        while (true)
                        {
                            ...
                            foreach(string key in keys)
                            {
                                ...
                                if (value != null && value.CurUpgradeInfo.IsOK == false && value.CurUpgradeInfo.sendTime != DateTime.MinValue)
                                {
