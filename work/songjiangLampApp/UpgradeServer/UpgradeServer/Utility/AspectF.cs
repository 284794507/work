...
namespace UpgradeServer.Utility
{
    /// <summary>
    ///AspectF 的摘要说明
    /// </summary>
    ...
    public static class AspectExtensions
    {
        ...
        [DebuggerStepThrough]
        public static AspectF Retry(this AspectF aspects, Action<Exception> errorHandler, Action retryFaild)
        {
            return aspects.Combine((work) =>
                Retry(0, 0, (error) => errorHandler(error), retryFaild, work));
        }

        [DebuggerStepThrough]
        public static AspectF Retry(this AspectF aspects, int retryDuration, int retryCount, Action retryFaild)
        {
            return aspects.Combine((work) =>
                Retry(retryDuration, retryCount, (error) => DoNothing(error), retryFaild, work));
        }
        ...
        [DebuggerStepThrough]
        public static AspectF Log(this AspectF aspect, Action<string,int> LogWriter, string beforeMessage, string afterMessage, int beforeLvl=2,int afterLvl=2)
        {
            return aspect.Combine((work) =>
            {
                if(!string.IsNullOrEmpty(beforeMessage))
                {
                    ...
                    LogWriter(beforeMessage,beforeLvl);
                }
                ...
                if (!string.IsNullOrEmpty(afterMessage))
                {
                    ...
                    LogWriter(afterMessage,afterLvl);
                }
            });
        }
    }
