...
using UpgradeServer.Utility;

namespace UpgradeServer.Controller
{
    [RoutePrefix("api/Upgrade")]
    public class UpgradeController:ApiController
    {
        ...
        public async Task<bool> UploadLamp()
        {
            bool result = true;
            if (Request.Content.IsMimeMultipartContent())
            {
                string path = Share.GetPath(Path.Combine(Environment.CurrentDirectory, "Lamp"));
        ...
        [HttpPost]
        [Route("UploadCabInfo")]
        public async Task<bool> UploadCab()
        {
            bool result = true;
            if (Request.Content.IsMimeMultipartContent())
            {
                string path = Share.GetPath(Path.Combine(Environment.CurrentDirectory, "Cab"));
                var provider = new MultipartFormDataStreamProvider(path);
                try
                {
                    await Request.Content.ReadAsMultipartAsync(provider);
